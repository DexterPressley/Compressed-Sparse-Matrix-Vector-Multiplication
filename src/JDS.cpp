...
static std::vector<unsigned int> per_row_nonzeros(const std::vector<std::vector<double>> &mat)
{
...
JDSMatrix to_jds(std::vector<std::vector<double>> mat)
{
    ...
    std::vector<std::vector<std::pair<unsigned int, double>>> rows(out.num_rows);
...
{
    std::vector<std::vector<double>> output(jds.num_rows, std::vector<double>(jds.num_cols, 0));
...
{
    std::vector<double> y(jds.num_rows, 0.0f);
    if (jds.num_rows == 0 || jds.num_cols == 0)
        return y;
